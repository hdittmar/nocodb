...
export function useCopyPaste({
  ...
  function isPasteable(row?: Row, col?: ColumnType, showInfo = false, avoidLtarRestrictions = false) {
    ...
    if (isVirtualCol(col) || isSystemColumn(col) || col?.readonly) {
      if (!avoidLtarRestrictions || (!isLinksOrLTAR(col) && !isDoc(col))) {
        if (showInfo) {
      ...
      if (clipboardData?.includes('\n') || clipboardData?.includes('\t') || isMultiCellSelection) {
        ...
        for (let i = 0; i < selectionRowCount; i++) {
          ...
          for (let j = 0; j < clipboardMatrix[clipboardRowIndex].length; j++) {
            ...
            if (isDoc(column) && isPasteable(targetRow, column, false, true)) {
              // Doc fields need per-cell API calls to duplicate the document
              const clipItem = extractCellClipboardData(storedCopiedData, clipboardRowIndex, j)
              const targetRowPk = extractPkFromRow(targetRow.row, meta.value?.columns as ColumnType[])
              if (clipItem?.dbCellValue && clipItem?.column?.id && clipItem?.rowId && targetRowPk && column.id) {
                const oldValue = targetRow.row[column.title!] || null
                try {
                  const result = await $api.internal.postOperation(
                    ...
                    meta.value?.base_id as string,
                    {
                      ...
                      sourceColumnId: clipItem.column.id,
                      sourceRowId: clipItem.rowId,
                      targetColumnId: column.id as string,
                      targetRowId: targetRowPk,
                    },
                  )
                  if (result?.id) {
                  ...
                  // silently skip failed duplications
                }
              }
            } else if (isPasteable(targetRow, column)) {
        ...
        if (pastedDocCells.length) {
          await syncCellData?.(activeCell.value, groupPath)

          addUndo({
            redo: {
              fn: async (cells: typeof pastedDocCells) => {
              ...
              fn: async (cells: typeof pastedDocCells) => {
                ...
                for (const cell of cells) {
                  ...
                  try {
                    ...
                    rowObj.row[cell.colTitle] = cell.oldValue
                  } catch { /* skip */ }
                }
                await syncCellData?.(activeCell.value, groupPath)
              },
        ...
        if (selection.value.isSingleCell()) {
          ...
          // Handle Doc field paste — duplicate the source document
          if (isDoc(columnObj)) {
            if (!isPasteable(rowObj, columnObj, true, true)) return
            ...
            if (pasteVal === undefined || !ncIsObject(pasteVal) || !pasteVal.rowId || !pasteVal.columnId) return
            ...
            try {
              const result = await $api.internal.postOperation(
                ...
                {
                  operation: 'docFieldDuplicate',
                  sourceColumnId: pasteVal.columnId,
                  sourceRowId: pasteVal.rowId,
                  targetColumnId: columnObj.id as string,
                  targetRowId: pasteRowPk,
                },
              )
                  ...
                  undo: {
                    fn: async (cellCtx: Cell, col: ColumnType, oldVal: any, docId: string) => {
                      ...
                      await $api.internal.postOperation(
                        meta.value?.fk_workspace_id as string,
                        meta.value?.base_id as string,
                        { operation: 'docFieldDelete', columnId: col.id, rowId: pasteRowPk },
                      )
          ...
          for (const row of rows) {
            ...
            for (const col of cols) {
              if (isDoc(col) && isPasteable(row, col, false, true)) {
                const clipItem = extractCellClipboardData(storedCopiedData, 0, 0)
                const targetRowPk = extractPkFromRow(row.row, meta.value?.columns as ColumnType[])
                if (clipItem?.dbCellValue && clipItem?.column?.id && clipItem?.rowId && targetRowPk && col.id) {
                  const oldValue = row.row[col.title!] || null
                  try {
                    const result = await $api.internal.postOperation(
                      meta.value?.fk_workspace_id as string,
                      meta.value?.base_id as string,
                      {
                        ...
                        sourceColumnId: clipItem.column.id,
                        sourceRowId: clipItem.rowId,
                        targetColumnId: col.id as string,
                        targetRowId: targetRowPk,
                      },
                    )
                    if (result?.id) {
          ...
          if (pastedDocCells2.length) {
            await syncCellData?.(activeCell.value, groupPath)

            addUndo({
              redo: {
                fn: async (cells: typeof pastedDocCells2) => {
                  const { cachedRows } = getDataCache(groupPath)
                  for (const cell of cells) {
                    const rowObj = cachedRows.value.get(cell.rowIndex)
                    if (!rowObj) continue
  ...
  async function clearCell(ctx: { row: number; col: number; path?: Array<number> } | null, skipUpdate = false) {
    ...
    if (
      ...
      (!isLinksOrLTAR(columnObj) && !isDoc(columnObj) && isVirtualCol(columnObj))
    ) {
      if (
        columnObj.readonly ||
        (isSystemColumn(columnObj) && !isLinksOrLTAR(columnObj)) ||
            ...
            redo: {
              fn: async (cellCtx: { row: number; col: number }, col: ColumnType) => {
                ...
                await $api.internal.postOperation(
                  meta.value?.fk_workspace_id as string,
                  meta.value?.base_id as string,
                  { operation: 'docFieldDelete', columnId: col.id, rowId: pk },
                )
